...
- **Single-instance route strata** — BAGFAŞ (N₂O-controlled fertilizer, n=1), Gübretaş (blender, n=1), Tosyalı Osmaniye (DRI-EAF, n=1). LODO over n=1 is structurally impossible to evaluate — the held-out facility has no in-stratum training data.
...
- **BF/BOF stratum has n=3 globally** (Erdemir, İsdemir, Kardemir — the only TR integrated mills). LODO over n=3 is trivially "predict from the other 2"; seed-to-seed variance on this stratum is large (one run İsdemir lands at 0.56×, another at 1.15×).
