...
**TR-MRV-Bench: A public per-facility emissions benchmark for Turkish CBAM-scope industry, with a physics baseline that beats the EU CBAM default by 81% across all four CBAM scopes**

## Abstract (~200 words)

CBAM and emerging emissions-trading regimes require per-facility CO₂ accounting that is independently verifiable. Existing per-facility data sources have known gaps: Climate TRACE under-reports the three Turkish BF/BOF steel mills in our sample by 20-30%; operator self-reports trust the operator; satellite tools (GHGSat, Carbon Mapper) focus on methane with no CBAM tie-in. We release **TR-MRV-Bench**, a public benchmark of 59 Turkish CBAM-scope facilities with three-tier supervision (**20 audit-grade strong labels** across all four CBAM scopes — cement, steel, aluminum, fertilizer — from operator IARs, TSRS-compliant sustainability reports, and KGK-verified ISO 14064-1 statements; 8 Climate TRACE per-asset labels; capacity-factor-corrected default labels for the rest). On this bench we evaluate a closed-form physics baseline — `capacity × emission-factor × capacity-factor`, with route-aware EF priorities (steel BF/BOF / EAF / DRI-EAF; aluminum primary / downstream; fertilizer integrated / N₂O-controlled / blender) and cf priorities defined in §3 — and find it reduces per-plant log-MAE by **81.2% vs the EU CBAM default** in leave-one-disclosure-out evaluation across the 20 audit-grade test facilities. A 2-layer neural network trained on the bench's residuals against this prior reaches **82.0%** single-run (5-outer-LODO 95% CI on a comparable n=18 subset: **81.4% ± 3.1%**). Ridge regression on the same features lags both at 78.7%. The actionable artifacts are the bench, the formula, and the source-cited disclosure crawl. The improvement is route-asymmetric: integrated big-emitter predictions (Erdemir 1.09×, İsdemir 1.04×, Kardemir 1.33×, Akçansa Çanakkale 0.84×, Batısöke 1.02×, Çolakoğlu 0.95×, İzdemir 0.97×) all land within ±35% of audit; single-instance strata (BAGFAŞ N₂O-controlled fertilizer 3.71×, Gübretaş blender 1.42×) are honest LODO failures since the model has never seen one in training. n=20 is still small and we are explicit about that in §8. The bench and code are released under Apache-2.0.
...
2. **cf-corrected formula baseline (§3, §5).** Closed-form `cap × EF × cf` with explicit route-aware EF and cf priority rules. **81.2% log-MAE reduction vs EU default on n=20 LODO** — predicts Erdemir Ereğli (1.09×), İsdemir (1.04×), Akçansa Çanakkale (0.84×), Batısöke (1.02×), Çolakoğlu (0.95×), İzdemir (0.97×) all within ±20% of audit. NN adds 0.8 pp.
...
5. **Negative result reversed at n=18-20 (§6).** A LoRA-shaped MLP, statistically tied with the closed-form physics formula at n=8, beats it by 1.8 pp at n=18 and 0.8 pp at n=20 (82.0% vs 81.2%). Ridge regression on the same features lags at 78.7%. The learned model's contribution is real but small; the formula carries ~99% of the signal. Future work should focus on data growth (more disclosures, satellite features) before parameter growth.
