...
CBAM and emerging emissions-trading regimes require per-facility CO₂ accounting that is independently verifiable. Existing per-facility data sources have known gaps. Independent estimators: **Climate TRACE** (satellite + bottom-up inventory) under-reports 4 of 5 audit-matched Turkish facilities (mean bias −17%, median −23%; under-reporting concentrated in steel and cement); **GHGSat / Carbon Mapper** focus on methane plumes and have no CBAM tie-in. Calculation engines (**CarbonChain**, **Persefoni**) are activity-based — they require operator data inputs and don't independently estimate facility emissions, so they're not comparable baselines for an arms-length verification problem. We release **TR-MRV-Bench**, a public benchmark of 59 Turkish CBAM-scope facilities with three-tier supervision (**21 audit-grade strong labels** across all four CBAM scopes — cement, steel, aluminum, fertilizer — from operator IARs, TSRS-compliant sustainability reports, and KGK-verified ISO 14064-1 statements; 8 Climate TRACE per-asset labels; capacity-factor-corrected default labels for the rest). On this bench we evaluate a closed-form physics baseline — `capacity × emission-factor × capacity-factor`, with route-aware EF priorities (steel BF/BOF / EAF / DRI-EAF; aluminum primary / downstream; fertilizer integrated / N₂O-controlled / blender) and cf priorities (Climate-TRACE per-asset > operator-disclosed-production / capacity > sector-mean default) defined in §3 — and find it reduces per-plant log-MAE by **85.3% vs the EU CBAM default** in leave-one-plant-out evaluation across the 21 audit-grade test facilities (capacities operator-audited). A 2-layer neural network trained on the bench's residuals against this prior reaches **+83.3% with 95% data-bootstrap CI [+72.0%, +90.6%]** (5000 resamples) — statistically tied with the closed-form formula. Ridge regression on the same features lags both at 75.6% (ridge overfits the noisy S5P NO₂ features; the formula and the NN handle them better). Per-sector 95% bootstrap CIs: cement [+66.2, +92.2], EAF steel [+96.6, +97.6], BF/BOF steel [-289.2, +97.2] (n=3 stratum, structurally wide), aluminum downstream [+89.9, +91.5], fertilizer [+34.6, +93.0]. The actionable artifacts are the bench, the formula, and the source-cited disclosure crawl. Most BF/BOF mills (Erdemir, İsdemir, Kardemir) sit within ±15% of the EU default because the EU integrated-steel default 1.9 t/t is already close to TR audited reality; cement and EAF are where the formula's value is concentrated. n=21 is still small and we are explicit about that in §8. The bench and code are released under Apache-2.0.
...
2. **cf-corrected formula baseline (§3, §5).** Closed-form `cap × EF × cf` with explicit route-aware EF and cf priority rules. **85.3% log-MAE reduction vs EU default on n=21 leave-one-plant-out** (capacities operator-audited) — 15 of 21 predictions land within ±20% of audit truth. NN ties at 83.6%.
...
4. **Climate TRACE under-reports TR industrial emissions (§5.2).** 4 of 5 audit-matched facilities under-reported: Erdemir −29%, İsdemir −22%, Kardemir −23%, Nuh −23%; Göltaş the lone over-report at +11%. Mean bias −17%, median −23%. Adding CT cf as a model feature worsens leave-one-plant-out accuracy by ~4 percentage points; this is why the no_ct ablation is the headline configuration.
5. **The formula and the model are statistically tied (§6).** A LoRA-shaped MLP achieves +83.3% vs the closed-form formula's 85.3% on n=21 leave-one-plant-out. Ridge regression on the same features lags at 81.4%. **The actionable shipped baseline is the formula**; the NN is a working reference implementation that opens room for future signal extraction (satellite features, multi-year leave-one-plant-out). At this data scale (~39 training samples after leave-one-plant-out holdout), parameter growth offers no meaningful gain over data growth.
...
**Statistical:**
- **n=21 audit-grade test facilities is still small.** Headline reduction 85.3% (formula B1) has a 95% data-bootstrap CI of [+72.0%, +90.6%] (5000 resamples of the n=21 facilities). The seed-level CI is tight (±0.3%) but the facility-bootstrap CI is wide — that's the honest read on benchmark size, not training noise.
- **Single-instance route strata** — BAGFAŞ (N₂O-controlled fertilizer, n=1), Gübretaş (blender, n=1), Tosyalı Osmaniye (DRI-EAF, n=1). leave-one-plant-out over n=1 is structurally impossible to evaluate — the held-out facility has no in-stratum training data.
...
- **BF/BOF stratum has n=3 globally** (Erdemir, İsdemir, Kardemir — the only TR integrated mills). leave-one-plant-out over n=3 is trivially "predict from the other 2"; seed-to-seed variance on this stratum is large (one run İsdemir lands at 0.56×, another at 1.15×).
- **Ablation matrix N_OUTER=1.** The 9-variant ablation (§6) uses 1 outer × 3 inner = 3 seeds per variant per facility. Small effects (≤2 pp, e.g., the no_s5p vs full margin) are within seed noise; a publishable ablation needs N_OUTER ≥ 5 (≈4 hours wall clock). Direction-of-contribution conclusions (prior +32 pp, route +18 pp, disclosed-cf +11 pp) are robust to this; small effects (CT, satellites) are not.
...
- **The Tosyalı Holding 425k 2022 Scope 1 is implausibly low** for a ~3M t/yr EAF operator and we flag it as `provenance=disputed` in the bench. It is not used as supervision for the headline metric (Tosyalı facilities are not in the disclosure leave-one-plant-out set).
