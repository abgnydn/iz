...
- ✅ **22-critique honesty pass + baselines + Erdemir capacity fix + CI bands (2026-05-26)** — Red-team review of 22 plausible reviewer critiques; fixed 18, honestly acknowledged 4. **Built `bin/baselines.py` comparing EU default / Climate TRACE direct / cf-corrected formula / ridge regression / iz-1 NN on same LODO split.** Headline finding: **the closed-form formula B1 (`cap × EF × cf`) achieves 87.1% reduction; iz-1 NN reaches 88.7% ± 2.4% (95% CI, 5 outer LODO runs).** Formula and NN are statistically indistinguishable — the formula is the simpler ship. **Fixed Erdemir Ereğli capacity 6M → 4M** (operator-published nameplate is 4M crude steel/yr; affected disc_cf 0.557 → 0.836 and EU default 11.4M → 7.6M). **Added `provenance` column to `tr_facility_known_emissions.csv`** distinguishing direct / allocated / derived / disputed. Marked Tosyalı Holding 425k as `disputed` (implausibly low). Dropped "ternary multi-modal Earth-observation foundation model" and "federated" buzzwords throughout; renamed paper to "TR-MRV-Bench: A public per-facility emissions benchmark for Turkish CBAM-scope industry, with a physics baseline that beats the EU CBAM default by 87%". Added Climate TRACE as explicit baseline B3 (37.4% on matched n=3 subset). Built `bin/e2e_lodo_with_ci.py` for proper mean ± 2σ CI reporting. Per-sector with CI: cement **88.1% ± 1.7%**, EAF **98.3% ± 3.3%**, BF/BOF **29.8% ± 35.3%** (wide because n=3 stratum). Updated README, PAPER_OUTLINE abstract, PAPER_METHOD, PAPER_DISCUSSION, paper preview HTML throughout. Detail in [[E42-iz-ct-label-divergence]] follow-up #10.
